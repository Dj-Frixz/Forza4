...
def brain(board,empty_cells,color):
    ...
    for i in indices:
        ...
        win = checkwin2.run(moved[0]==color,moved[1],i)
        if win==-2: return i
    ...
    if simm==1:
        ...
        for i in indices:
            moved = move(board,empty_cells,i,color)
            choices[i] = think(moved[0],moved[1],(color%2)+1,deepness+1,checkwin2.run(moved[0]==color,moved[1],i))
    elif simm==0:
        choices = np.zeros(indices.size,np.int8)
        for i in indices:
            moved = move(board,empty_cells,i,color)
        ...
        for i in indices:
            moved = move(board,empty_cells,i,color)
            choices[i] = concentrate(moved[0],moved[1],(color%2)+1,deepness+1,checkwin2.run(moved[0]==color,moved[1],i))
    return choices
...
def think(board,empty_cells,color,deepness,forced=-1):
    ...
    if simm==1:
        ...
        for i in indices:
            moved = move(board,empty_cells,i,color)
            choices[i] = checkwin2.run(moved[0]==color,moved[1],i)

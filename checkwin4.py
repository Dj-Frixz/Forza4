...
def run(board, empty_cells, b):
    ...
    if win[0]!=2:
        win = win + np.array(horizontal(board[a],a,empty_cells==a+1))
    #top-left to bottom-right diagonal win/tris check
    if win[0]!=2:
        win = win + np.array(firstDiagonal(np.diag(board,b-a),b-a,empty_cells))
    #top-right to bottom-left diagonal win/tris check
    if win[0]!=2:
        win = win + np.array(secondDiagonal(np.diag(board[:,::-1],6-a-b)[::-1],6-a-b,empty_cells))
    if win[0]==0:
        return -1       #nothing
    elif win[0]==1:
